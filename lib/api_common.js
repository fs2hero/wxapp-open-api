...
class API {
  ...
  async request(url, opts, retry) {
    ...
    const requestSpeedHook = this.hooks.requestSpeed.bind(this, this.appid)
    var options = {}
   ...
   * ```
   */
  async getAccessToken(noLock = false) {
    const componentAccessToken = await this.getComponentAccessToken(noLock)
    const url = this.prefix + 'component/api_authorizer_token?component_access_token=' +
      componentAccessToken.componentAccessToken
    let unlock
    if(!noLock) {
      unlock = await this.acquiredLocker()
      if (!unlock) {
        throw new Error(
    ...
    try {
      ...
      if(!authorizer_refresh_token) {
        unlock && unlock()
        throw new Error(`${this.appid} authorizer_refresh_token is empty`)
      }
      const params = {
      ...
      const data = await this.request(url, options, 0)
   ...
   * ```
   */
  async getComponentAccessToken(noLock = false) {
